...
def ARDB(name, x, filter_size, ratio, n_feats):

    x0 = x

    x1 = tf.nn.relu(Conv(name + '_conv1_3', x, filter_size, n_feats, n_feats, 1))
    x2 = Conv(name + '_conv2_3', x1, filter_size, n_feats, n_feats, 1)

    x3 = tf.concat([x2, x1], axis=3)

    x3 = Conv(name + '_conv3_3', x3, 1, 2*n_feats, n_feats, 1)

    s = NL_CA(name, x, ratio, n_feats)
    x3 = tf.multiply(x3, s)

    x3 = Conv(name + '_conv4_3', x3, filter_size, n_feats, n_feats, 1)
    x3 = tf.concat([x3, x2, x1], axis=3)
    x3 = Conv(name + '_conv5_3', x3, 1, 3*n_feats, n_feats, 1)
...
def Residual_Group(name, x, n_ARDB, filter_size, ratio, n_feats):

    x_n = []
    skip = x

    x1 = ARDB(name+'_1', x, filter_size, ratio, n_feats)

    for i in range(1, n_ARDB):
        x = ARDB(name+'_'+str(i+1), x, filter_size, ratio, n_feats)
        x_n.append(x)
        x1 = tf.concat([x_n[i-1], x1], axis=3)

    x = Conv(name + '_conv_tail', x1, 1, n_ARDB*n_feats, n_feats, 1)

    out = x + skip

    return out

def All_Residual_Group(name, x, n_ARDG, n_ARDB, filter_size, ratio, n_feats):

    x_n = []

    x1 = Residual_Group(name+'_1', x, n_ARDB, filter_size, ratio, n_feats)

    for i in range(1, n_ARDG):
        x = Residual_Group(name+'_'+str(i+1), x, n_ARDB, filter_size, ratio, n_feats)
        x_n.append(x)
        x1 = tf.concat([x_n[i-1], x1], axis=3)

    x = Conv(name + '_conv_tail', x1, 1, n_ARDG*n_feats, n_feats, 1)

    out = x

    return out

def NL_CA(name, x, ratio, n_feats):

    x1 = tf.nn.relu(Conv(name + '_conv1_1', x, 1, n_feats, n_feats, 1))
    x1 = Conv(name + '_conv2_1', x1, 1, n_feats, n_feats, 1)

    s = Attention(name, x1, ratio, n_feats)

    return s

def Attention(name, x, ratio, n_feats):
