...
def train(args, model, sess):
    ...
    if args.fine_tuning :  # load pre-trained model
        ...
        if args.load_tail_part:
            ...
            variables_to_restore = [var for var in tf.global_variables() if 'up_sample' not in var.name and 'conv_rec' not in var.name and 'learning_step' not in var.name]
    ...
    if args.in_memory:
        ...
        while count < args.max_step:
            ...
            for k in range(step):
                ...
                _, losses = sess.run([model.train, model.loss], feed_dict = {model.LR : lr_batch, model.GT : gt_batch})
                count = sess.run(model.global_step)

                e_time = time.time()

                print("%d training step completed， Loss : %0.4f，  Elpased time : %0.4f。" % (count, losses, (e_time - s_time)))
                ...
                if ((count) % args.model_save_freq ==0):
                    saver.save(sess, os.path.join(args.model_path,'ARDN_X%d_%d_%d_%d'%(args.scale, args.n_feats, args.n_ARDG, args.n_ARDB)),global_step = count, write_meta_graph = False)
    
        saver.save(sess, os.path.join(args.model_path,'ARDN_X%d_%d_%d_%d_last'%(args.scale, args.n_feats, args.n_ARDG, args.n_ARDB)),global_step = count, write_meta_graph = False)
